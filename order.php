  ...
  <div id="content">  
	...
	<div id="menu">
     <table border="2" align="center" width="800"> 
		...
		$no=1;
		$view= $mysqli->query("SELECT * FROM pesanan_customer");
		while ($a=mysqli_fetch_array($view)) {
		echo "<tr><td>$a[no_pesanan]</td>
				<td>$a[kode_produk]</td>
				<td>$a[nama_member]</td>
				<td>$a[jumlah_transfer]</td>
				<td>$a[nama_bank]</td>
				<td>$a[keterangan]</td>
				<td>$a[tanggal]</td>
				<td><a href='detailpesanan.php'>Detail</a></td>"; 
		$no++;
		}
		echo"</tr></table>";
			?>
